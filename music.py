...
class YTDLSource(discord.PCMVolumeTransformer):
    ...
    async def create_source(cls, ctx, search: str, *, loop, download=False):
        ...
        embed = discord.Embed(
            title="",
            description=
            f"Queued [{data['title']}]({data['webpage_url']}) [{ctx.author.mention}]",
...
class Music(commands.Cog):
        ...
        if not channel:
            try:
                ...
                embed = discord.Embed(
                    title="",
                    description=
                    "No channel to join. Please call `,join` from a voice channel.",
                    color=discord.Color.green(),
                )
                await ctx.send(embed=embed)
        ...
        if pos == None:
            ...
            try:
                ...
                embed = discord.Embed(
                    title="",
                    description=
                    f"Removed [{s['title']}]({s['webpage_url']}) [{s['requester'].mention}]",
                    color=discord.Color.green(),
                )
                await ctx.send(embed=embed)
            except:
    ...
    async def queue_info(self, ctx):
        ...
        upcoming = list(
            itertools.islice(player.queue._queue, 0,
                             int(len(player.queue._queue))))
        fmt = "\n".join(
    ...
    async def now_playing_(self, ctx):
        ...
        embed = discord.Embed(
            title="",
            description=
            f"[{vc.source.title}]({vc.source.web_url}) [{vc.source.requester.mention}] | `{duration}`",
            color=discord.Color.green(),
        )
    ...
    @commands.command(name="volume",
                      aliases=["vol", "v"],
                      description="changes Kermit's volume")
